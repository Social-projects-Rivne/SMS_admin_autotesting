...
class ExtendedTeachersModel(object):
    ...
    select_teachers_query='SELECT t.id as id, t.name as name, t.login as login, t.email as email, t.password as password, t.role_id as       					role_id, t.school_id as school_id, \
    ...
    def set_password_to_teacher_by_id(self, id_, password):
        orm=self.initORM()
        orm.update('Teachers', 'password=%d'%(password_id), 'id=%d'%(id_))
        orm.close()
