...
class TestExtendedSchoolsModel(unittest.TestCase):
    def setUp(self):
        ...
        results = self.orm.mysql_do(
            ...
            ' where name = "%s"' % self.school_to_test.name)
        for row in results:
    ...
    def tearDown(self):
        ...
        try:
            self.orm.delete('Schools', 'name = "%s" and address = "%s"'
                       % (self.test_school_name, self.test_school_address))
            self.orm.delete('Schools', 'name = "%s" and address = "%s"'
    ...
    def test_update_school_by_id(self):
        ...
        results = self.orm.mysql_do(
            ExtendedSchoolsModel.select_schools_query + ' where id = %d'
